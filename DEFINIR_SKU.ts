function main(workbook: ExcelScript.Workbook) {
    ...
    for (let linhaAtual = 1; linhaAtual <= ultimaLinhaInicial;) {
      ...
      const valorDate = w_inputInicial.getCell(linhaAtual, 10).getValue();
      ...
      if (valorColunaE === "Instore") {
        ...
        // Data
        w_online.getCell(1, colOnline).setValue(valorDate);
        // w_online.getCell(1, colOnline).getFormat().getFont().setBold(true);s
        w_online.getCell(1, colOnline).getFormat().getFill().setColor('e8e8e8');
